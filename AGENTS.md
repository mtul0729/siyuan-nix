...
- Client packaging: electron-builder runs with `--dir`, kernel symlinked as `SiYuan-Kernel`. When upstream changes `app/` layout or `InitPandoc`, diff against the nixpkgs `pkgs/by-name/si/siyuan` package as a reference for what changed. The pandoc zip is still generated, but only as a placeholder: electron-builder's `extraResources` names it and `app/scripts/afterPack.js` throws `Packaged Pandoc archive not found` without it (it only checks that the extracted `bin/pandoc` is a non-empty regular file). The extracted copy is unused — the shared kernel uses nix pandoc via `set-pandoc-path.patch` — so `installPhase` overwrites `resources/pandoc/bin/pandoc` with a store symlink. That's a deliberate ~209 MiB dedupe, not a bug (the path is kept so the kernel's built-in-pandoc fallback stays valid). Don't "optimize" the placeholder zip away: the build breaks without it.
