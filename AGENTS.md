...
- Client packaging: electron-builder runs with `--dir`, kernel symlinked as `SiYuan-Kernel`. When upstream changes `app/` layout or `InitPandoc`, diff against the nixpkgs `pkgs/by-name/si/siyuan` package as a reference for what changed. We deliberately do NOT rebuild `pandoc/pandoc-*.zip` the way nixpkgs does: upstream's `extraResources` and `app/scripts/afterPack.js` are a self-consistent pair pointing at upstream's own vendored zip, so that zip just flows through as a throwaway intermediate (and nothing in our scripts may depend on its internal layout). `installPhase` then replaces the extracted `resources/pandoc/` with a store symlink, because the shared kernel uses nix pandoc via `set-pandoc-path.patch`. That's a deliberate ~209 MiB dedupe, not a bug.
